...
proc qc::date_year_iso_start {date} {
    ...
    return [cast date [clock scan "3 days ago" -base [clock scan "Thursday" -base [clock scan "$year-01-01"] ]]]
}
...
proc qc::date_year_iso_end {date} {
    ...
    return [cast date [clock scan "4 days ago" -base [clock scan "Thursday" -base [clock scan "$next_year-01-01"]]]]
}

proc qc::date_iso_year { date } {
    #| Return the ISO year.
    return [qc::cast integer [clock format [cast epoch $date] -format "%G"]]
}
...
proc qc::date_iso_week_start {date} {
	...
	return [cast date [clock scan "last monday" -base [cast epoch $date]]]
    }
}

proc qc::date_iso_week_end {date} {   
    #| Returns the date of the end of the week in which $date falls
    return [cast date [clock scan "sunday" -base [cast epoch $date]]]
}

proc qc::date_iso_week { date } {
    #| Return the ISO week number.
    return [qc::cast integer [clock format [cast epoch $date] -format "%V"]]
}

proc qc::date_yesterday { date } {
    #| Return yesterday's date
    return [cast date "$date -1day"]
}
...
proc qc::date_month { date } {
    ...
    if { [regexp {^(\d{4})-(\d{2})-(\d{2})$} $date -> year month day] } {
        return [qc::cast integer $month]
    }
    return [qc::cast integer [clock format [cast epoch $date] -format "%m"]]
}

proc qc::date_doy { date } {
    #| Return the day of year number.
    return [qc::cast integer [clock format [cast epoch $date] -format "%j"]]
}
...
proc qc::date_year { date } {
    ...
    return [clock format [cast epoch $date] -format "%Y"]
}

proc qc::date_dom { date } {
    #| Return the day of the month.
    if { [regexp {^(\d{4})-(\d{2})-(\d{2})$} $date -> year month day] } {
        return [qc::cast integer $day]
    }
    return [qc::cast integer [clock format [cast epoch $date] -format "%d"]]
}

proc qc::date_dow { date } {
    #| Return the day of the week.
    return [qc::cast integer [clock format [cast epoch $date] -format "%u"]]
}

proc qc::date_day_name { date } {
    #| Return the full day of the week as Monday,Tuesday etc
    return [clock format [cast epoch $date] -format "%A"]
}
...
proc qc::date_month_shortname { date } {
    ...
    return [clock format [cast epoch $date] -format "%b"]
}
...
proc qc::date_compare { date1 date2 } {  
    ...
    set epoch1 [cast epoch $date1]
    set epoch2 [cast epoch $date2]
    if { $epoch1 > $epoch2 } {
	return 1
...
proc qc::date_quarter {date} {
    ...
    return [qc::cast integer [expr {ceil(double([date_month $date])/3)}]]
}

proc qc::date_quarter_start {date} {
    #| Return the date at the beginning of the quarter
    set year  [date_year $date]
    set month [qc::cast integer [expr {ceil(double([date_month $date])/3)*3-2}]]
    return [cast date "$year-$month-01"]
}

proc qc::date_quarter_end {date} {
    #| Return the date at the end of the quarter
    set year  [clock format [cast epoch $date] -format %Y]
    set month [qc::cast integer [expr {ceil(double([date_month $date])/3)*3}]]
    return [cast date [clock scan "+1 month -1 day" -base [clock scan "$year-$month-01"]]]
}

proc qc::dates {from_date to_date} {
    #| Return a list of dates from $from_date to $to_date inclusive.
    set dates {}
    set date [cast date $from_date]
    while {[date_compare $date $to_date]<1} {
	lappend dates $date
	set date [cast date "$date +1 day"]
    }
    return $dates
}
...
proc qc::date_days {from_date to_date} {
    ...
    return [qc::round [expr {([cast epoch $to_date]-[cast epoch $from_date])/(60.0*60*24)}] 0]
}
