doc authentication {
	...
	The proc [doc_link session_exists] can be used to test if the session is valid and [doc_link auth_session] can be used to return the employee_id of the session owner.
	...
	The proc [doc_link auth_hba_check] can be used to test if the user can be established based on IP address. The proc [doc_link auth_hba] can be used to return the employee_id associated with the IP address of the [doc_link conn_remote_ip "current connection"] based on the employee table.
	...
	The proc [doc_link auth_password_check] can be used to test if the pair of values is valid and [doc_link auth_password] can be used to return the employee_id associated with this pair of values from the employee table.
	<h3>General</h3>
	The proc [doc_link auth_check] tries to establish if we can authenticate who the current user/employee is by checking session based authentication first and then host based authentication.
	<p> 
	The proc [doc_link auth] returns the employee_id of the current user by checking session based authentication first and then host based authentication. 
    }
}
